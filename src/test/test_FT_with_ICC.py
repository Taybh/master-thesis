...
def compute_ICC21(x, y):
    assert (len(x) == len(y)), "gt and prediction have different lengths"
    ...
    # create numpy array of sets
    M = np.stack((x, y), axis=1).astype(np.float)

    # compute BMS = MSR in matlab: MSR = var(mean(M, 2)) * k;
    meansM = np.mean(M, axis=1)
    BMS = np.var(meansM, ddof=1)
    BMS = BMS * k

    # compute JMS = MSC in matlab: MSC = var(mean(M, 1)) * n;
    Mxy = np.mean(M, axis=0)
    Mvar = np.var(Mxy, ddof=1)
    JMS = Mvar * n

    # compute total Sum of Squares, SStot = SStotal in matlab: SSTotal = var(M(:)) *(n*k - 1); with var(M(:)) is the variance of the whole set
    WholeSet = np.concatenate((x, y), axis=0).astype(np.float)
    SStot = np.var(WholeSet, ddof=1) * (n * k - 1)

    # compute EMS = MSE in matlab: MSE = (SStotal - MSR *(n - 1) - MSC * (k -1))/ ((n - 1) * (k - 1));
    EMS = (SStot - BMS * (n - 1) - JMS * (k - 1)) / ((n - 1) * (k - 1))

    # compute icc2_1 = ICC A-1 in matlab: r = (MSR - MSE) / (MSR + (k-1)*MSE + k*(MSC-MSE)/n); with MSR = BMS MSC = JMS and MSE = EMS
...
def cr(data, lc_mask_left, lc_mask_right, ref_mask_left, ref_mask_right):
    ...
    try:
        ...
        if np.mean(lc_mask_left)==0:
            lc_left = np.zeros(shape= (639,639,383), dtype=np.float32)
        if np.mean(lc_mask_right)==0:
            lc_right = np.zeros(shape=(639,639,383), dtype=np.float32)
        med_lc_left = np.median(lc_left)
        med_lc_right = np.median(lc_right)
        max_lc_left = np.max(lc_left)
        max_lc_right = np.max(lc_right)
...
def test_fold(test_idx, counter): #,target):
    ...
    net.cuda()
    checkpoint = torch.load(model_dir + 'f' + str(counter) + '_wdh' +str(num_wdh)+ '/checkpoint_' + model_type)
...
'''
        ...
        for v in range(len(cr_lists['median_left'])):
            icc_csv.write(str(tid) + ',' + str(counter)+',' + str(cr_gt_lists['median_left'][v]) + ',' + str(cr_lists['median_left'][v]) + ',' +
                          str(cr_gt_lists['median_right'][v]) + ',' + str(cr_lists['median_right'][v]) + ',' +
                          str(cr_gt_lists['max_left'][v]) + ',' + str(cr_lists['max_left'][v]) + ','+
                          str(cr_gt_lists['max_right'][v]) + ',' + str(cr_lists['max_right'][v]) + '\n')
            #df = pd.read_csv(results_dir + os.sep + origin + '_CRs_ICCs' + t + '.csv', sep=',')
...
if __name__ == '__main__':
    pattern_total = ['last', 'up2', 'up1', 'up0', 'down3', 'down2', 'down1', 'whole']
    lr_type_total = ['decay', 'freeze']
    target = 'sn' # 'lc'
    origin = 'LcToSn'#'SnToLc'
    patchsize = 128
    ...
    #print(indices)
    indices_22 = sample(indices_total, 22)
    indices_42 = sample(indices_total, 42)
    indices_62 = sample(indices_total, 62)

    train_val_22, test_22 = train_test_split(indices_22, test_size=0.27, shuffle=True,
                                             random_state=131294)
    train_val_42, test_42 = train_test_split(indices_42, test_size=0.27, shuffle=True,
                                             random_state=131294)
    train_val_62, test_62 = train_test_split(indices_62, test_size=0.27, shuffle=True,
                                             random_state=131294)
    print(('test22:', test_22))
    print(('test42:', test_42))
    print(('test62:', test_62))
    ...
    for num in num_samples:
            for typ in lr_type_total:
                for patt in pattern_total:
                    #indices = indices_total

                    model_dir = 'logs/unet' + str(num)+os.sep+ origin + os.sep + typ+ os.sep + patt + os.sep
                    #model_name = 'lc_Scratch'


                    # create results directory
                    results_dir = 'EVAL_TL' + str(num)+'_' +typ +'_' + patt
                    ...
                    #dsc_csv = open(results_dir + os.sep + origin + '_DSC.csv', 'w')
                    #dsc_csv.write('Subject,fold,DSC_left,DSC_right,DSC_all\n')


                    #cr_csv = open(results_dir + os.sep + origin + '_CRs_automated.csv', 'w')
                    #cr_csv.write('Subject,fold,CR_median_left,CR_median_right,CR_max_left,CR_max_right\n')


                    #cr_gt_csv = open(results_dir + os.sep + origin + '_CRs_manual.csv', 'w')
                    #cr_gt_csv.write('Subject,fold,CR_median_left,CR_median_right,CR_max_left,CR_max_right\n')

                    cr_types = ['median_left', 'median_right', 'max_left', 'max_right']

                    icc_csv = open(results_dir + os.sep + origin + '_CRs_ICCs' + '.csv', 'w')
                    icc_csv.write('Subject, fold,gt_median_left,auto_median_left,gt_median_right,auto_median_right,'
                                  'gt_max_left,auto_max_left,gt_max_right,auto_max_right\n')

                    #icc_csv = open(results_dir + os.sep + origin + '_CRs_ICCs_conf-' + str(conf_for_icc) + '.csv', 'w')
                    ...
                    for i in range(num_folds):
                        print('number of samples:'+str(num)+'  origin:'+ origin +'  lr_type:'+ typ + '  pattern:' + patt +'  fold:'+str(i))
