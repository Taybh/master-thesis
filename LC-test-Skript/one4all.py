...
def load_pred_masks(index):
    lc_pred_left = nib.load(test_lc_dir + os.sep + str(index) + '_0_' + model_name + '.nii.gz').get_data()
    lc_pred_right = nib.load(test_lc_dir + os.sep + str(index) + '_1_' + model_name + '.nii.gz').get_data()
...
def load_original_data(index):
    ...
    for r, d, f in os.walk(orig_data_dir):
        for x in f:
            if 'left' not in x and 'right' not in x and 'MB' not in x \
                    and not x.startswith('.') \
                    and not x.endswith('txt'):
                img_files.append(x)
    img_files = sorted(img_files)
    return nib.load(orig_data_dir + os.sep + img_files[index]).get_data()
...
def load_gt_ref(index):
    ref_masks = np.load(gt_ref_dir + os.sep + 'lc_ref_{:02d}.npz'.format(index))['ref']
    return ref_masks[0], ref_masks[1]
...
if __name__ == '__main__':
    test_name = 'healthy_pp-bound+tresh_R2_'
    orig_data_dir = '/home/max/git/LCSN-Seg/data/lc/BETTS82'
    gt_lc_dir = '/home/max/git/LCSN-Seg/data/lc/BETTS82_Rater2'#'/home/max/git/LCSN-Seg/data/lc_processed'
    gt_ref_dir = '/home/max/git/LCSN-Seg/data/lc_refs'
    model_name = 'LC_03_'
    test_lc_dir = '/home/max/git/LCSN-Seg/results/LC_03-masks'
    test_ref_dir = '/home/max/git/LCSN-Seg/data/lc_refs'
    num_of_samples = 82
    conf_for_icc = 0.95
    write_pp_masks = True

    orig_data = load_original_data
    gt_lc = load_gt_lc_r2#load_gt_lc#load_gt_lc_r2
    gt_ref = load_gt_ref
    test_lc = load_pred_masks
    test_ref = load_gt_ref
